...
from app.schemas.product import ProductResponse, ProductCreate, ProductUpdate, ProductList
...
async def get_product(id: UUID, db: AsyncSession = Depends(get_db)):
    product = await product_service.get_product_by_id(db, id)
    if not product:
...
async def update_product(
    ...
    product = await product_service.update_product(db, id, update)
    if not product:
        raise HTTPException(status_code=404, detail='Product not found')
    return product
